...
session's platform + user_id from the environment (injected by Hermes), looks
up the user's google_id in user_mappings.json, then copies their personal
google_token.json into the shared HERMES_HOME root where google_api.py reads it.

Usage:
    python kosha_auth.py --platform discord --user-id 1036621467502252164
    python kosha_auth.py --check   (just verify current token is valid)
...
def swap_token(platform: str, user_id: str) -> int:
    """Copy this user's token to the shared root. Returns exit code."""
    ...
    google_id  = mappings[key]["google_id"]
    token_path = user_token_path(google_id)

    if not token_path.exists():
        # Fall back to the auth.json store
        ...
        if auth_path.exists():
            ...
            if token:
                ...
                print(f"OK: Token swapped for {key} (google_id={google_id}) via auth.json")
                return 0
